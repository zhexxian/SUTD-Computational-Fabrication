...
function fem_2d_beam() 

%--- Input parameters

% Material parameters 
E  = 5e9;          % Pa = N/m²  (Young’s modulus)
nu = 0.4;          % (Poisson’s ratio)
thickness = 0.5;    % m

% Traction force
P = -5e6;    % N/m (on the boundary)
...
forceVec(rightBoundary+numberNodes) = P * thickness * nodeSpacingY;
forceVec(rightBoundary(1)+numberNodes) = P * thickness * nodeSpacingY/2;
forceVec(rightBoundary(end)+numberNodes) = P * thickness * nodeSpacingY/2;



% Essential boundary conditions
fixedNodeX = find(nodeCoordinates(:,1)==0); % fixed U where X==0
fixedNodeY = find(nodeCoordinates(:,1)==0); % fixed V where X==0
